...
def assemble_line(line, pc, labels):
    ...
    # CMP has two explicit operands and writes its -1/0/+1 result to the
    # architecture's implicit condition register r5.
    if mnemonic == 'CMP':
        if len(tokens) != 3:
            raise ValueError("CMP syntax is: CMP lhs, rhs/#imm")
        encoded = []
        for operand in tokens[1:3]:
            if operand.startswith('#'):
                value = parse_imm(operand)
                if not (0 <= value <= TYPE1_IMM_MAX):
                    raise ValueError(
                        f"Immediate #{value} out of range for CMP "
                        f"(must be 0–{TYPE1_IMM_MAX})")
                encoded.append((1, value))
            else:
                encoded.append((0, parse_reg(operand)))
        return encode_type1(T1_OPS['CMP'], encoded[0][0], encoded[0][1],
                            encoded[1][0], encoded[1][1], 5)
