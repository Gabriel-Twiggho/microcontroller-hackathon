...
module test_harness;
    // The full demo keeps its PID loop at instruction address 0x000E.
    // Stop after 1,000 completed updates; MAX_CYCLES remains a separate
    // safety timeout for a stalled or incorrectly assembled program.
        ...
        while (!dbg_halt && !performance_done && cycle_count < max_cycles) begin
            ...
            if (dbg_pc == PID_LOOP_PC) begin
                if (loops == 0) begin
                    ...
                    $display("Average cycles per PID update: %0.3f",
                             $itor(cycle_count - start_cycle) /
                             PID_ITERATIONS);
...
`ifdef DISPLAY_ACTUATOR
        $display("Raw PWM output [0x011C] = %0d (0x%08h)",
                 u_cpu.u_dmem.mem[16'h011C >> 2],
                 u_cpu.u_dmem.mem[16'h011C >> 2]);
`endif
        $finish;
    end
