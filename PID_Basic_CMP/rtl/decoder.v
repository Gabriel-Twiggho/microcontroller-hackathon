...
// Combinational decoder for the PID_Basic_CMP instruction formats.
module decoder (
    ...
    always @* begin
        ...
        if (type1) begin
            case (type1_opcode)
                ...
                `T1_CMP: begin
                    // CMP has two encoded operands and an implicit r5 result.
                    dest = `COND_REG_ADDR;
                    compare = 1'b1;
                    register_write = 1'b1;
                    halt = 1'b0;
            ...
            case (type2_opcode)
                ...
                `T2_JZ: begin
                    src_a = `COND_REG_ADDR;
                    jump = 1'b1;
                    branch_condition = `BR_ZERO;
                    halt = 1'b0;
                end
                `T2_JNZ: begin
                    src_a = `COND_REG_ADDR;
                    jump = 1'b1;
                    branch_condition = `BR_NOT_ZERO;
                    halt = 1'b0;
                end
                `T2_JLT: begin
                    src_a = `COND_REG_ADDR;
                    jump = 1'b1;
                    branch_condition = `BR_LESS;
                    halt = 1'b0;
                end
                `T2_JGT: begin
                    src_a = `COND_REG_ADDR;
                    jump = 1'b1;
                    branch_condition = `BR_GREATER;
                    halt = 1'b0;
                end
                `T2_LI: begin
